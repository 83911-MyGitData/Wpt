...
app.post('/users/reg',(req,res)=>{
    ...
    let queryTxt = `insert into user (firstName,lastName,email,password,phoneNumber,createdTimestamp) values ('${firstName}','${lastName}',
    '${email}','${changedPassword}','${phoneNumber}',CURRENT_TIMESTAMP)`;

    connection.query(queryTxt,(err,r)=>{
...
app.post('/users/login',(req,res)=>{
    ...
    connection.query(queryText,(err,r)=>{
        if(!err)
            {
                const id = r[0].id;
                const token = jwt.sign({changedPassword,id},secret);
                res.write(JSON.stringify(r));
...
app.use((req,res,next)=>{
    ...
    if(token!=undefined)
        {
            let dataInside = jwt.verify(token,secret);
            next();
        }
        else{
            res.write("send token");
        }
})
...
app.post('/booking',(req,res)=>{
    ...
    const token = req.headers.authorization;
    const dataOfToken = jwt.verify(token,secret);
...
app.listen(9998,()=>{console.log("happy")});